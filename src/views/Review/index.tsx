...
const TableDiff = ({ changeType, changedColor, colorDiff, oldColors }) => (
  <>
    ...
    <table>
      <thead>
        <tr>
          ...
          {colorDiff[changeType][changedColor] ?
            Object.keys(colorDiff[changeType][changedColor]).map(colorTone => (
      ...
      <tbody>
        <tr>
          <td className={changeType === "deleted" ? "deleted-color" : "added-color"}>
          ...
          {colorDiff[changeType][changedColor] ?
            Object.keys(colorDiff[changeType][changedColor]).map(colorTone =>
              <td
                ...
                style={{
                  backgroundColor: colorDiff[changeType][changedColor][colorTone]
                }}>{colorDiff[changeType][changedColor][colorTone]}</td>
            ) :
            Object.keys(oldColors[changedColor]).map(colorTone =>
              <td
                key={colorTone}
                ...
                }}>{oldColors[changedColor][colorTone]}</td>
            )
          }
        </tr>
        {changeType === "updated" && (<tr>
          <td className={`${changeType}-color`}>
            Old Color
          </td>
          {Object.keys(colorDiff[changeType][changedColor]).map(colorTone => (
            <td
...
const Diff = ({ changeType, colorDiff, oldColors }) => (
  <>
    ...
    {Object.keys(colorDiff[changeType]).map(changedColor => (
      <TableDiff
        key={changedColor}
    ...
    <>
      ...
      <div id="review-panel">
        {Object.keys(colorDiff).map(changeType => (
          <Diff key={changeType} changeType={changeType} colorDiff={colorDiff} oldColors={oldColors} />
        ))}
      </div>
    </>
  )
